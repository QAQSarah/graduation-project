	...
	<div class="x-body">
		<div class="layui-row">
			<form class="layui-form layui-col-md12 x-so" >
				...
				<div style="float: left; width: 40%">
					<lable> <b>作者:</b></lable>
					<br> <input type="text" id="sreachEmail" placeholder="请输入文章作者"
						autocomplete="off" class="layui-input" style="width: 90%" />
				</div>
			</form>
			<div style="float: right; margin: 50px;">
					<button class="layui-btn">
						<a href="javascript:void(0)"  onclick="sreachUser();"><i class="layui-icon">&#xe615;</i></a>
					</button>
			</div>
		</div>
		<xblock>
		<button class="layui-btn"
			onclick="x_admin_show('添加文章','article/toAddArticle.do',1000,600)">
					...
					<tr>
						...
						<td>${article.createtime }</td>
							<td class="td-manage"> <a title="查看"
								onclick="x_admin_show('查看','showPage.do?id='+${article.id},1000,600)"
								href="javascript:;"> <i class="layui-icon">&#xe615;</i><a title="编辑"
								onclick="x_admin_show('编辑','/it_web/article/toEditArticle.do?id='+${article.id},1000,600)"
								href="javascript:;"> <i class="layui-icon">&#xe642;</i>
							</a> <a title="删除" onclick="member_del(this,${article.id})"
								href="javascript:;"> <i class="layui-icon">&#xe640;</i>
							</a></td>
					</tr>
				</c:forEach>
	...
	<script>
		...
		function member_del(obj, id) {
			layer.confirm('确认要删除吗？', function(index) {
				//发异步删除数据
				$.post("/it_web/article/deleteArticle.do",{id:id},function(){
									});
		...
		function showByPage(id){
			cid=$(".cid").val();
			$.post("/it_web/article/articleList.do?pageNum="+id,function(data){
				$("#tab文章列表").html(data);
				})
		}
		function showByPagePrevious(pageNum){
			...
			cid=$(".cid").val();
			if(num>=1){
				$.post("/it_web/article/articleList.do?pageNum="+num,function(data){
					$("#tab文章列表").html(data);
					})
			}else{
				confirm("当前已经是最前页");
		...
		function sreachUser(){
			...
			$.post("/it_web/article/articleList.do",{
				articleName:name,
				articleAuthor:exmail,
		}, function(data) {
			$("#tab文章列表").html(data);
			});
