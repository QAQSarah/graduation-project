...
<body class="layui-anim layui-anim-up">
	<div class="x-body">
		...
		<table class="layui-table">
			...
			<tbody>
				<c:forEach items="${reponseVos}" var="reponse">
					<tr>
						<td hidden="hidden">
						...
						<td>${reponse.fromUser}</td>
						<td>${reponse.createtime }</td>
						<td>${reponse.commentContext }</td>
						<td class="td-manage"><a title="删除"
							onclick="member_del(this,${reponse.id})" href="javascript:;">
								<i class="layui-icon">&#xe640;</i>
						</a></td>
					</tr>
				</c:forEach>
	...
	<script>
		...
		function member_del(obj, id) {
			layer.confirm('确认要删除吗？', function(index) {
				//发异步删除数据
				$.post("/it_web/deleteReponse.do",{id:id},function(data){
					layer.msg(data, {
						icon : 1,
						time : 1000
					});
		...
		function showByPage(id){
			cid=$(".cid").val();
			$.post("/it_web/reponseList.do?pageNum="+id,function(data){
				$("#tab回复数据列表").html(data);
				})
		}
		function showByPagePrevious(pageNum){
			...
			cid=$(".cid").val();
			if(num>=1){
				$.post("/it_web/reponseList.do?pageNum="+num,function(data){
					$("#tab回复数据列表").html(data);
					})
			}else{
				confirm("当前已经是最前页");
