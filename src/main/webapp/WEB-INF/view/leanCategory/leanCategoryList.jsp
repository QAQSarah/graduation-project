...
<body class="layui-anim layui-anim-up">
	<div class="x-body">
		...
		<xblock>
		<button class="layui-btn"
			onclick="x_admin_show('添加类别','line/toAddLeanCategory.do',600,600)">
				...
				<c:forEach items="${categorys}" var="category">
					<tr>
						...
						<td class="td-manage"><a title="编辑"
							onclick="x_admin_show('编辑','/it_web/line/toEditLeanCategory.do?id='+${category.id},1000,600)"
	...
	<script>
		...
		function member_del(obj, id) {
			layer.confirm('确认要删除吗？', function(index) {
				//发异步删除数据
				$.post("/it_web/line/deleteLeanCategory.do",{id:id},function(data){
							layer.msg(data, {
								icon : 1,
								time : 1000
							});
							$.post("/it_web/line/leanCategoryList.do", function(data) {
		...
		function showByPage(id){
			cid=$(".cid").val();
			$.post("/it_web/line/leanCategoryList.do?pageNum="+id,function(data){
				$("#tab学习资源类别列表").html(data);
				})
		}
		function showByPagePrevious(pageNum){
			...
			cid=$(".cid").val();
			if(num>=1){
				$.post("/it_web/line/leanCategoryList.do?pageNum="+num,function(data){
					$("#tab学习资源类别列表").html(data);
					})
			}else{
				confirm("当前已经是最前页");
		...
		function sreachUser(){
			...
			$.post("/it_web/line/leanCategoryList.do",{
				name:name,
		}, function(data) {
			$("#tab学习资源类别列表").html(data);
			});
