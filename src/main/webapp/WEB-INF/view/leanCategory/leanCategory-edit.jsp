...
<body class="layui-anim layui-anim-up">
		...
		<div class="layui-row">
			<div style="float: left; width: 45%; margin-right: 5%">
				...
				<br><input hidden="hidden" value="${category.id }" id="Cid"/> <input type="text" id="sreachName" placeholder="请输入分类名称"
					autocomplete="off" class="layui-input" style="width: 100%" value="${category.name }">
			</div>
			<div style="float: left; width: 45%; margin-right: 5%">
				<lable> <b>简介:</b></lable>
				<br><input type="text" id="sreachIntroduction" placeholder="请输入简介"
					autocomplete="off" class="layui-input" style="width: 100%" value="${category.introduce }">
			</div>
		</div>
		<div>
	...
	<script>
		...
		$("#tjbutton").click(function() {
			...
			var introduce=$("#sreachIntroduction").val();
			var id=$("#Cid").val();
			$.post("/it_web/line/editLeanCategory.do", {
				name : title,
				id:id,
				introduce:introduce
			}, function(data) {
