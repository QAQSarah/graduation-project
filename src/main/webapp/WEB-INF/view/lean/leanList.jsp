...
<body class="layui-anim layui-anim-up">
	<div class="x-body">
		...
		<xblock>
		<button class="layui-btn"
			onclick="x_admin_show('添加资源','line/toAddLean.do',600,600)">
			...
			<tbody>
				<c:forEach items="${categorys}" var="category">
					<tr>
						...
						<td>${category.name }</td>
						<td>${category.downloadurl }</td>
						<td>${category.leanlineCategory }</td>
						<td class="td-manage"><a title="编辑"
							onclick="x_admin_show('编辑','/it_web/line/toEditLean.do?id='+${category.id},1000,600)"
	...
	<script>
		...
		function member_del(obj, id) {
			layer.confirm('确认要删除吗？', function(index) {
				//发异步删除数据
				$.post("/it_web/line/deleteLean.do",{id:id},function(data){
							layer.msg(data, {
								icon : 1,
								time : 1000
							});
							$.post("/it_web/line/leanList.do", function(data) {
								$("#tab学习资源列表").html(data);
							});
				});
			});
		...
		function showByPage(id){
			cid=$(".cid").val();
			$.post("/it_web/line/leanList.do?pageNum="+id,function(data){
				$("#tab学习资源列表").html(data);
				})
		}
		function showByPagePrevious(pageNum){
			...
			cid=$(".cid").val();
			if(num>=1){
				$.post("/it_web/line/leanList.do?pageNum="+num,function(data){
					$("#tab学习资源列表").html(data);
					})
			}else{
				confirm("当前已经是最前页");
